...
class Property(models.Model):
    ...
    def save(self,*args,**kwargs):
        if not self.reference:
            ...
            self.reference=f'FAST-BIEN-{self.pk:06d}'
            return super().save(update_fields=['reference'])
        return super().save(*args,**kwargs)
    @property
    def match_score(self):
        score=70
        score += 10 if self.security else 0
        score += 10 if self.water else 0
        score += 5 if self.electricity else 0
...
class Contract(models.Model):
    ...
    def save(self,*args,**kwargs):
        if not self.reference:
            super().save(*args,**kwargs)
            self.reference=f'FAST-CTR-{self.created_at.year if self.created_at else 2026}-{self.pk:06d}'
            return super().save(update_fields=['reference'])
        return super().save(*args,**kwargs)
