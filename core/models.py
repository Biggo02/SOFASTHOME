...
class Property(models.Model):
    ...
    furniture_details=models.TextField(blank=True)
    furnished_bedrooms=models.PositiveIntegerField(default=0)
    furnished_salons=models.PositiveIntegerField(default=0)
    ...
    water_days_per_week=models.PositiveIntegerField(default=7)
    water_source=models.CharField(max_length=30,choices=WATER_SOURCES,blank=True)
    water_details=models.TextField(blank=True)
    electricity=models.BooleanField(default=True)
    electricity_days_per_week=models.PositiveIntegerField(default=7)
    electricity_source=models.CharField(max_length=30,choices=ELECTRICITY_SOURCES,blank=True)
    electricity_details=models.TextField(blank=True)
    floor_type=models.CharField(max_length=30,choices=FLOOR_TYPES,blank=True)
    ceiling_type=models.CharField(max_length=30,choices=CEILING_TYPES,blank=True)
    condition=models.CharField(max_length=120,blank=True)
    furnished_type=models.CharField(max_length=100,blank=True)
...
class Visit(models.Model):
    ...
    preferred_date=models.DateField(null=True,blank=True); preferred_time=models.TimeField(null=True,blank=True)
    scheduled_date=models.DateField(null=True,blank=True); scheduled_time=models.TimeField(null=True,blank=True)
    owner_approved=models.BooleanField(default=False); agent_approved=models.BooleanField(default=False)
    agent=models.ForeignKey(User,on_delete=models.SET_NULL,null=True,blank=True,related_name='assigned_visits')
    observation=models.TextField(blank=True); status=models.CharField(max_length=20,choices=STATUS,default='pending'); comment=models.TextField(blank=True); created_at=models.DateTimeField(auto_now_add=True)
...
class Payment(models.Model):
    ...
    contract=models.ForeignKey(Contract,on_delete=models.CASCADE,related_name='payments'); amount_due=models.DecimalField(max_digits=10,decimal_places=2); amount_paid=models.DecimalField(max_digits=10,decimal_places=2,default=0); due_date=models.DateField(); paid_date=models.DateField(null=True,blank=True); reference=models.CharField(max_length=80,blank=True); status=models.CharField(max_length=20,choices=STATUS,default='upcoming')
