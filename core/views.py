...
def require_verified(request, action):
    if is_identity_verified(request.user):
        return True
    messages.warning(request, f'Votre identité doit être validée par FASTHOME avant {action}.')
    return False
...
def search(request):
    ...
    if q:
        ...
        for term in q.replace('-', ' ').split():
            query |= Q(title__icontains=term) | Q(province__icontains=term) | Q(city__icontains=term) | Q(commune__icontains=term)
        qs = qs.filter(query)
...
def add_property(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if submitting and not require_verified(request, 'soumettre une publication'):
...
def verification_upload(request):
    if request.method == 'POST':
        for kind in ('id_front', 'id_back', 'selfie'):
            ...
            if uploaded:
                VerificationDocument.objects.create(user=request.user, kind=kind, file=uploaded); audit(request, 'verification.document_uploaded', details={'kind': kind})
