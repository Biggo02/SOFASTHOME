...
def match_score(prop, request):
    """Legacy score kept for compatibility with older internal templates.

    Public matching is handled by core.search_views and uses the seven
    authoritative criteria. This helper deliberately uses only the
    province/city/commune hierarchy and never a neighbourhood field.
    """
    score = 0
    ...
    if q:
        hay = f'{prop.title} {prop.province} {prop.city} {prop.commune}'.lower().replace('-', ' ')
        score += 25 if q.replace('-', ' ') in hay else 15 if any(
            part in hay for part in q.replace('-', ' ').split()
        ) else 5
    else:
        score += 20
    ...
    try:
        requested_bedrooms = int(bedroom_value or 0)
    except (TypeError, ValueError):
        requested_bedrooms = 0
...
def search(request):
    """Legacy search endpoint retained only for compatibility.

    The public /rechercher/ route points to core.search_views.search, which
    implements the current seven-criteria matching flow.
    """
    qs = Property.objects.filter(status='published').prefetch_related('images')
    ...
    for prop in properties:
        prop.ui_score = match_score(prop, request)
    properties.sort(key=lambda p: p.ui_score, reverse=True)
    return render(
        request,
        'search.html',
        {'properties': properties, 'q': q, 'types': Property.TYPES},
    )
...
def property_detail(request, pk):
    ...
    if request.GET.get('matching') == '1' or request.GET.get('from_matching') == '1':
        context['score'] = match_score(prop, request)
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        user_for_auth = User.objects.filter(
            Q(username__iexact=identifier) | Q(email__iexact=identifier)
        ).first()
        user = authenticate(
            request,
            username=user_for_auth.username if user_for_auth else identifier,
            password=request.POST.get('password', ''),
        )
        if user:
            login(request, user)
            audit(request, 'account.login', user)
            return redirect('dashboard')
...
def dashboard(request):
    ...
    return render(
        ...
        'dashboard.html',
        {
            ...
            'visits': Visit.objects.filter(requester=user)
            .select_related('property')
            .order_by('-created_at')[:5],
            'contracts': Contract.objects.filter(user=user).select_related('property'),
            'payments': Payment.objects.filter(contract__user=user)
...
def publications(request):
    return render(
        ...
        'list.html',
        {
            ...
            'items': Property.objects.filter(owner=request.user).order_by('-updated_at'),
            'kind': 'property',
        },
    )
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if obj.status == 'review':
            Notification.objects.create(
                ...
                message=f'{obj.reference} a été transmise à FASTHOME.',
            )
        messages.success(
            request,
            'Publication soumise à vérification.'
            if obj.status == 'review'
...
def upload_property_images(request, pk):
    ...
    if existing + len(files) > 10:
        messages.error(request, 'Un bien peut contenir au maximum 10 photos.')
        return redirect('publications')
    for i, uploaded in enumerate(files):
        PropertyImage.objects.create(
            ...
            image=uploaded,
            order=existing + i,
            is_cover=(existing == 0 and i == 0),
        )
    audit(request, 'property.images_uploaded', prop, {'count': len(files)})
...
def request_visit(request, pk):
    ...
    if request.method == 'POST':
        ...
        Notification.objects.create(
            ...
            message=f'Votre demande pour {prop.title} est en attente de validation.',
        )
        Notification.objects.create(
            user=prop.owner,
            title='Nouvelle demande de visite',
            message=f'La demande #{visit.pk} concerne votre bien {prop.reference}.',
        )
        audit(request, 'visit.requested', visit)
...
def visits(request):
    return render(
        ...
        'list.html',
        {
            ...
            'items': Visit.objects.filter(requester=request.user)
            .select_related('property')
            .order_by('-created_at'),
            'kind': 'visit',
        },
...
def contracts(request):
    return render(
        ...
        {
            'title': 'Mes contrats',
            'items': Contract.objects.filter(user=request.user)
            .select_related('property')
            .order_by('-created_at'),
            'kind': 'contract',
        },
...
def payments(request):
    return render(
        ...
        {
            'title': 'Mes paiements',
            'items': Payment.objects.filter(contract__user=request.user)
            .select_related('contract__property')
            .order_by('due_date'),
            'kind': 'payment',
        },
...
def payment_proof(request, pk):
    ...
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        audit(request, 'payment.proof_uploaded', payment, {'proof_id': proof.pk})
        messages.success(request, 'Preuve de paiement envoyée à FASTHOME.')
...
def notifications(request):
    ...
    return render(
        request,
        'list.html',
        {'title': 'Notifications', 'items': qs, 'kind': 'notification'},
    )
    ...
    return render(
        ...
        'list.html',
        {
            ...
            'items': Property.objects.filter(
                pk__in=request.session.get('favorites', []),
                status='published',
            ),
            'kind': 'favorite',
        },
...
def toggle_favorite(request, pk):
    get_object_or_404(Property, pk=pk, status='published')
    ...
    return HttpResponseRedirect(
        request.META.get('HTTP_REFERER') or redirect('property_detail', pk=pk).url
    )
...
def messages_page(request):
    return render(
        ...
        'placeholder.html',
        {
            ...
            'text': 'Contactez uniquement FASTHOME. Les coordonnées privées des propriétaires restent masquées.',
        },
    )
...
# These legacy functions remain callable for backward compatibility. The URLconf
# uses dedicated modules for the public About/How-it-works/Contact pages.
def about(request):
    return render(
        request,
        'placeholder.html',
        {
            'title': 'À propos',
            'text': 'FASTHOME simplifie la recherche, la visite et la gestion locative en RDC.',
        },
    )
...
def how_it_works(request):
    return render(
        ...
        'placeholder.html',
        {
            ...
            'text': 'Recherchez, comparez, demandez une visite, recevez la confirmation, visitez, puis suivez votre location depuis un seul compte.',
        },
    )
...
def contact(request):
    return render(
        ...
        {
            'title': 'Contact',
            'text': 'L’équipe FASTHOME vous accompagne à chaque étape.',
        },
    )
...
def verification_upload(request):
    if request.method == 'POST':
        ...
        messages.success(
            request,
            'Vos documents de vérification ont été envoyés à FASTHOME.',
        )
