...
def search(request):
    ...
    q=request.GET.get('q','').strip(); ptype=request.GET.get('type','')
...
def dashboard(request):
    user=request.user
    links=[('⌂ Tableau de bord','/espace-personnel/'),('⌕ Rechercher','/rechercher/'),('♡ Mes favoris','#'),('▣ Mes publications','/mes-publications/'),('◷ Mes demandes de visite','/mes-visites/'),('▤ Mes contrats','/mes-contrats/'),('◉ Mes paiements','/mes-paiements/'),('◴ Mes échéances','/mes-echeances/'),('● Notifications','/notifications/'),('⚙ Mon profil','#')]
    return render(request,'dashboard.html',{'links':links,'properties':Property.objects.filter(owner=user),'visits':Visit.objects.filter(requester=user).order_by('-created_at')[:5],'contracts':Contract.objects.filter(user=user),'payments':Payment.objects.filter(contract__user=user).order_by('due_date')[:5],'notifications':Notification.objects.filter(user=user).order_by('-created_at')[:5]})

@login_required
def publications(request): return render(request,'list.html',{'title':'Mes publications','items':Property.objects.filter(owner=request.user),'kind':'property'})
@login_required
...
@login_required
def contracts(request): return render(request,'list.html',{'title':'Mes contrats','items':Contract.objects.filter(user=request.user).select_related('property'),'kind':'contract'})
@login_required
...
@login_required
def notifications(request): return render(request,'list.html',{'title':'Notifications','items':Notification.objects.filter(user=request.user).order_by('-created_at'),'kind':'notification'})
@login_required
def messages_page(request): return render(request,'placeholder.html',{'title':'Messagerie','text':'Votre messagerie FASTHOME est prête pour les échanges sécurisés avec l’agence.'})
