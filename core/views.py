...
def home(request): return render(request,'home.html',{'properties':Property.objects.filter(status='published').order_by('-created_at')[:8],'types':Property.TYPES})

def search(request):
    qs=Property.objects.filter(status='published'); q=request.GET.get('q','').strip(); ptype=request.GET.get('type',''); bedrooms=request.GET.get('bedrooms',''); commune=request.GET.get('commune','').strip()
    if q:
        query=Q()
...
def register(request):
    ...
    if request.method=='POST' and form.is_valid():
        user=form.save(commit=False); user.set_password(form.cleaned_data['password']); user.save(); login(request,user); messages.success(request,'Bienvenue sur FASTHOME. Votre compte unique est prêt.'); return redirect('dashboard')
...
def toggle_favorite(request,pk):
    ...
    request.session['favorites']=ids; ref=request.META.get('HTTP_REFERER'); return HttpResponseRedirect(ref) if ref else redirect('property_detail',pk=pk)
@login_required
...
def review_publication(request,pk):
    ...
    if request.method=='POST':
        ...
        prop.save(update_fields=['status','rejection_reason','updated_at']); Notification.objects.create(user=prop.owner,title='Statut de publication mis à jour',message=f'{prop.reference} : {prop.get_status_display()}.'); messages.success(request,notice); return redirect('admin_dashboard')
...
def manage_visit(request,pk):
    ...
    if request.method=='POST':
        status=request.POST.get('status',visit.status); visit.status=status
        visit.agent_approved=request.POST.get('agent_approved')=='on'; visit.owner_approved=request.POST.get('owner_approved')=='on'
        if visit.agent_approved and visit.owner_approved and status=='pending': visit.status='confirmed'
        visit.scheduled_date=request.POST.get('scheduled_date') or visit.preferred_date; visit.scheduled_time=request.POST.get('scheduled_time') or visit.preferred_time; visit.observation=request.POST.get('observation',''); visit.save()
