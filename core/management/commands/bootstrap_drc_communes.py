...
def explicitly_commune(tags):
    values = [clean(tags.get(key)).casefold() for key in ("designation", "official_status", "government", "place", "type", "admin_type", "boundary")]
    text = " ".join(values)
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic(), connection.cursor() as cursor:
            ...
            for relation in level7:
                ...
                urban = is_inside_city(geometry, city_geometries)
            ...
            cursor.execute("""
                WITH candidates AS (
                    SELECT c.id AS commune_id, p.name AS province_name, t.name AS city_name,
                    ...
                    SELECT *, ROW_NUMBER() OVER (
                        PARTITION BY commune_id ORDER BY p_rank,t_rank,p_overlap DESC NULLS LAST,t_overlap DESC NULLS LAST,city_name
                    ) AS rn FROM candidates
                )
                UPDATE core_administrativearea c
                   SET province_name=COALESCE(r.province_name,''), city_name=COALESCE(r.city_name,''), updated_at=NOW()
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Communes importées={imported}, explicites={explicit_count}, déduites par inclusion dans une vraie ville={spatial_count}, "
            f"ignorées={skipped}, admin_level=7 non retenues={non_communes}, rattachées={attached}"
        ))
        ...
        self.stdout.write(self.style.WARNING(
            "QUALITÉ: selon les conventions OSM RDC, admin_level=7 est une commune en zone urbaine, mais une collectivité/secteur/chefferie en zone rurale. "
            "Cette importation refuse donc les admin_level=7 ruraux non explicitement identifiés comme communes. La couche reste cartographique et ne constitue pas un registre juridique exhaustif."
        ))
