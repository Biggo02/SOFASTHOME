...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--levels", default="ADM1,ADM2", help="Niveaux geoBoundaries à importer, par défaut ADM1,ADM2. ADM3 peut être ajouté séparément.")
        parser.add_argument("--simplified", action="store_true", help="Utilise la géométrie simplifiée pour réduire la taille et accélérer l'import.")
    ...
    def _import_features(self, level, data, metadata, clear):
        ...
        imported = skipped = spatial_parented = 0
        ...
        with transaction.atomic(), connection.cursor() as cursor:
            ...
            if level == "ADM2":
                # Use a correlated scalar subquery. PostgreSQL rejects references to
                # the target UPDATE alias from a LATERAL subquery in this position.
                cursor.execute(
                    """
                    UPDATE core_administrativearea AS child
                    SET province_name = (
                        SELECT p.name
                        FROM core_administrativearea AS p
                    ...
                    WHERE child.level = 'territory'
                      AND EXISTS (
                        SELECT 1
                        FROM core_administrativearea AS p2
                        WHERE p2.level = 'province'
                          AND ST_Intersects(child.geom, p2.geom)
                    )
                    """
                )
