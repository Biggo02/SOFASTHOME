...
class PropertyForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'commune': forms.TextInput(attrs={'placeholder': 'Commune'}),
            'neighborhood': forms.TextInput(attrs={'placeholder': 'Quartier'}),
            'avenue': forms.TextInput(attrs={'placeholder': 'Avenue / rue / boulevard'}),
            'number': forms.TextInput(attrs={'placeholder': 'Numéro (ex. 12A)'}),
            ...
            'bedrooms': forms.NumberInput(attrs={'min': 0}), 'salons': forms.NumberInput(attrs={'min': 0}), 'kitchens': forms.NumberInput(attrs={'min': 0}), 'bathrooms': forms.NumberInput(attrs={'min': 0}), 'toilets': forms.NumberInput(attrs={'min': 0}), 'shower_count': forms.NumberInput(attrs={'min': 0}),
            'rent': forms.NumberInput(attrs={'min': 1, 'step': '1', 'placeholder': 'Ex. 500000 FC'}), 'deposit': forms.NumberInput(attrs={'min': 0, 'step': '1', 'placeholder': 'Ex. 100000 FC'}),
        }
        labels = {
            ...
            'bedrooms': 'Nombre de chambres', 'salons': 'Nombre de salons', 'kitchens': 'Nombre de cuisines', 'bathrooms': 'Nombre de salles de bain', 'toilets': 'Nombre de toilettes', 'rent': 'Loyer mensuel (FC)', 'deposit': 'Garantie / dépôt (FC)', 'max_occupants': 'Nombre maximum d’habitants',
    ...
    def clean_room_details_json(self):
        ...
        except (TypeError, ValueError, json.JSONDecodeError): raise forms.ValidationError('Les caractéristiques détaillées des pièces sont invalides.')
        if not isinstance(value, list): raise forms.ValidationError('Les caractéristiques détaillées des pièces sont invalides.')
        return value[:100]

    def clean_photos(self):
        files = self.files.getlist('photos')
        if len(files) > 10: raise forms.ValidationError('Maximum 10 photos pour un bien.')
        for uploaded in files:
            if not uploaded.content_type.startswith('image/'): raise forms.ValidationError('Seules les images sont acceptées.')
        return files
