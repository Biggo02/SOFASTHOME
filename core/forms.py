...
class RegisterForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        profile_data = {
            'postnom': self.cleaned_data.get('postnom', '').strip(),
...
class PropertyForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {'description': forms.Textarea(attrs={'rows': 4, 'placeholder': 'Décrivez le bien, son environnement et ses particularités…'}), 'full_address': forms.TextInput(attrs={'placeholder': 'Adresse exacte — non affichée publiquement'}), 'availability_date': forms.DateInput(attrs={'type': 'date'}), 'water_days_per_week': forms.NumberInput(attrs={'min': 0, 'max': 7}), 'electricity_days_per_week': forms.NumberInput(attrs={'min': 0, 'max': 7}), 'max_occupants': forms.NumberInput(attrs={'min': 1, 'max': 100}), 'bedrooms': forms.NumberInput(attrs={'min': 0}), 'salons': forms.NumberInput(attrs={'min': 0}), 'kitchens': forms.NumberInput(attrs={'min': 0}), 'bathrooms': forms.NumberInput(attrs={'min': 0}), 'toilets': forms.NumberInput(attrs={'min': 0}), 'shower_count': forms.NumberInput(attrs={'min': 0}), 'rent': forms.NumberInput(attrs={'min': 1, 'step': '1', 'placeholder': 'Ex. 500000 FC'}), 'deposit': forms.NumberInput(attrs={'min': 0, 'step': '1', 'placeholder': 'Ex. 100000 FC'})}
