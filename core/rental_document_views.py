...
def _make_pdf(title, lines):
    ...
    for text in lines:
        ...
        if line:
            ...
            y -= 15
        if y < 70:
            c.showPage(); y = height - 55; c.setFont('Helvetica', 9)
...
def _property_lines(prop):
    return [
        ...
        f'Sécurité : {"Oui" if prop.security else "Non"}', f'Parking : {"Oui" if prop.parking else "Non"} — places : {prop.parking_spaces}',
...
def prepare_four_rental_documents(case):
    prop = case.property; owner_name = case.owner.get_full_name() or case.owner.username; tenant_name = case.tenant.get_full_name() or case.tenant.username; visit = case.visit
    common = [
        f'Dossier : {case.reference}', f'Bien : {prop.title} — {prop.reference}', f'Localisation : {prop.commune} — {prop.city} — {prop.province}',
        f'Adresse : {prop.full_address or "Non précisée"}', f'Constat : {visit.scheduled_date or visit.preferred_date or "À préciser"} à {visit.scheduled_time or visit.preferred_time or "À préciser"}',
        f'Propriétaire : {owner_name}', f'Locataire : {tenant_name}', '', 'CARACTÉRISTIQUES PRÉREMPLIES DU BIEN', *_property_lines(prop), '',
        f'Observation de visite : {visit.observation or "Aucune observation enregistrée."}', '', 'ÉTAT CONTRADICTOIRE À CONFIRMER',
        ...
        'Réserves / observations complémentaires : _______________________', '__________________________________________________________________',
    ]
    specs = [
    ...
    for doc_type, title, prefix, signature in specs:
        ...
        # Ne jamais écraser un fichier signé déjà téléversé, qu'il soit à vérifier, validé ou refusé.
        if not doc.file:
            ...
            doc.status = 'prepared'; doc.notes = 'Document prérempli par FASTHOME, prêt à signature.'; doc.save()
...
def _serve_pdf(request, case, title, filename, lines, party_id):
    if not _allowed(request, case): return HttpResponseForbidden('Accès refusé.')
    if not request.user.is_staff and request.user.pk != party_id: return HttpResponseForbidden('Ce document est réservé à la partie concernée.')
...
def rental_support_document_pdf(request, pk, document):
    ...
    prop = case.property; owner = case.owner.get_full_name() or case.owner.username; tenant = case.tenant.get_full_name() or case.tenant.username; visit = case.visit
    common = [
        f'Référence : {"EDL-PRO" if document == "owner_pv" else "EDL-LOC"}-{case.reference}', f'Dossier : {case.reference}', f'Bien : {prop.title} — {prop.reference}',
        f'Localisation : {prop.commune} — {prop.city} — {prop.province}', f'Adresse : {prop.full_address or "Non précisée"}', f'Propriétaire : {owner}', f'Locataire : {tenant}',
        f'Constat : {visit.scheduled_date or visit.preferred_date or "À préciser"} à {visit.scheduled_time or visit.preferred_time or "À préciser"}', '', 'CARACTÉRISTIQUES DU BIEN', *_property_lines(prop), '',
        'ÉTAT CONTRADICTOIRE À CONFIRMER', 'Murs / peinture : _______________________________________________', 'Sols : ___________________________________________________________',
        'Portes / fenêtres / serrures : _________________________________', 'Installations eau / sanitaires : ______________________________', 'Installation électrique : _______________________________________',
        'Mobilier / équipements : ________________________________________', 'Compteurs et relevés : __________________________________________', 'Clés remises : ___________________________________________________',
        f'Observations de visite : {visit.observation or "Aucune observation enregistrée."}', '',
    ]
    if document == 'owner_pv': return _serve_pdf(request, case, 'ÉTAT DES LIEUX — FASTHOME ↔ PROPRIÉTAIRE', f'EDL-PRO-{case.reference}.pdf', common + ['Partie signataire : Propriétaire', 'Signature du propriétaire : ______________________________', 'Signature / visa FASTHOME : ______________________________'], case.owner_id)
    if document == 'tenant_pv': return _serve_pdf(request, case, 'ÉTAT DES LIEUX — FASTHOME ↔ LOCATAIRE', f'EDL-LOC-{case.reference}.pdf', common + ['Partie signataire : Locataire', 'Signature du locataire : _________________________________', 'Signature / visa FASTHOME : ______________________________'], case.tenant_id)
